...
export class Enemy {
  constructor(type, tx, ty) {
    ...
    const stats = {
      frostling: { hp: 2, damage: 1, speed: ENEMY_SPEED * 0.6 },
      snowbat:   { hp: 1, damage: 1, speed: ENEMY_SPEED * 1.4 },
      guard:     { hp: 4, damage: 2, speed: ENEMY_SPEED },
      titan:     { hp: 14, damage: 3, speed: ENEMY_SPEED * 0.8 },
    };
    const s = stats[type] || stats.frostling;
    this.hp = s.hp;
    this.maxHp = s.hp;
  ...
  update(map, player) {
    ...
    this.moveTimer++;
    const changeInterval = this.type === 'snowbat' ? 30 : 60;

    if (this.type === 'titan') {
      this.chasePlayer(map, player, 0.7);
    } else if (this.type === 'snowbat') {
...
export function spawnDrop(type, x, y) {
  const drops = {
    frostling: [{ type: 'crystal_shard', chance: 0.4, value: 1 }],
    snowbat:   [{ type: 'heart', chance: 0.2 }],
    guard:     [{ type: 'crystal_shard', chance: 0.5, value: 3 }, { type: 'heart', chance: 0.15 }],
    titan:     [{ type: 'crystal_shard', chance: 1, value: 50 }],
  };
