/**
 * Linux-Desktop-Bridge für den NachtBlau Hub (Electron).
 * Gleiche API wie site-bridge.js / android-bridge.js — gleicher UI-Stand.
 */
(function initLinuxBridge() {
  ...
  window.launcher = {
      ...
      const games = gamesConfig.games.map((g) => {
        ...
        return {
          ...
          installInfo: {
            ...
            details: g.web?.note || g.web?.label || (isPaidContent ? "Kostenlos" : "Lokal / Web"),
          },
          unlocked: true,
        };
      });
    ...
    getPreviewUrl: async (gameId) => {
      ...
      if (game.web?.playUrl) {
        return { mode: "cover", hint: "Spiel starten — öffnet im Desktop-Fenster" };
      }
    ...
    launchGame: async (gameId) => {
      ...
      if (ipcRenderer) {
        await ipcRenderer.invoke("open-game-window", absUrl, gameId);
        return { mode: "window", url: absUrl };
      }
      window.open(absUrl, `nachtblau_${gameId}`);
      return { mode: "window", url: absUrl };
    },

    updateGame: async (gameId) => {
      if (ipcRenderer) {
        return ipcRenderer.invoke("update-game", gameId);
      }
      return { success: false, error: "Update nur in der gepackten Linux-App." };
    },

    openFolder: async (gameId) => {
      if (ipcRenderer) return ipcRenderer.invoke("open-folder", gameId);
    },

    openUrl: (url) => {
      if (ipcRenderer) ipcRenderer.invoke("open-external", url);
  ...
  if (label) label.textContent = "Linux Desktop";
})();
