...
grep -q 'ID=fedora' "$ROOT/system_files/usr/libexec/silk/brand-os-release" && ok "brand ID=fedora (BIB-kompatibel)" || bad "brand ID=fedora"
grep -q 'VARIANT_ID=silk' "$ROOT/system_files/usr/libexec/silk/brand-os-release" && ok "brand VARIANT_ID=silk" || bad "brand VARIANT_ID"
grep -q 'detect_fedora_version_id\|VERSION_ID=' "$ROOT/system_files/usr/libexec/silk/brand-os-release" && ok "brand keeps Fedora VERSION_ID" || bad "brand VERSION_ID"
test -f "$ROOT/system_files/usr/lib/systemd/system/silk-brand.service" && ok "silk-brand.service" || bad "silk-brand.service"
grep -q 'systemctl enable silk-brand.service' "$ROOT/build_files/05-finalize.sh" && ok "finalize enables silk-brand" || bad "finalize silk-brand"
grep -q 'localhost/silk-bib' "$(cd "$ROOT/.." && pwd)/.github/workflows/silk-disk.yml" && ok "disk workflow local BIB image rewrite" || bad "disk BIB local rewrite"
grep -q 'VERSION_ID=' "$(cd "$ROOT/.." && pwd)/.github/workflows/silk-disk.yml" && ok "disk workflow patches VERSION_ID for BIB" || bad "disk VERSION_ID patch"
grep -q 'volume_id = "SILK_X86_64"' "$ROOT/disk_config/iso.toml" && ok "ISO volume_id SILK" || bad "ISO volume_id"
grep -q 'publisher = "Nachtblau Crew"' "$ROOT/disk_config/iso.toml" && ok "ISO publisher Silk" || bad "ISO publisher"
! grep -qi 'aurora' "$ROOT/system_files/usr/share/silk/hardware-pc.txt" && ok "hardware-pc ohne aurora" || bad "hardware-pc aurora"
